  ...
  return (
    <div className="relative min-h-screen flex flex-col">
        ...
        <div className="flex items-center gap-4 text-sm">
          {loggedIn ? (
            ...
            <>
              ...
              <Link to="/signup" className="font-mono text-xs font-bold bg-mint text-[#04221a] px-4 py-2 rounded">
                GET STARTED
              </Link>
            </>
          )}
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center px-6 text-center max-w-3xl mx-auto">
        ...
        <div className="font-mono text-xs text-ice tracking-[0.18em] mb-3">EVERY SOURCE · ONE CALENDAR</div>
        ...
        <div className="flex items-center gap-3 mb-16">
          <Link to={primaryTo}>
            <Btn variant="mint" cls="!px-7 !py-3.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full text-left">
          {features.map((f) => (
            <HudBox key={f.title} className="p-5">
              <Icon name={f.icon} size={22} className="text-ice mb-2.5" />
              <h3 className="font-display font-bold text-[#e8f4f8] text-base mb-1">{f.title}</h3>
